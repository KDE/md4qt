...
inline long long int findRawTextIndex(std::shared_ptr<Paragraph<Trait>> p,
                                      long long int index)
{
    long long int res = -1;
...
template<class Trait>
inline long long int findKnownOpener(std::shared_ptr<Paragraph<Trait>> p,
                                     ...
                                     long long int openerIndex)
{
    ...
    const auto virginPos = po.m_fr.m_data[o.m_line].first.virginPos(0)
        + po.m_rawTextData[findRawTextIndex(p, o.m_index)].m_pos
        + o.m_endColumn;

    for (long long int i = 0; i < static_cast<long long int>(knownOpeners.size()); ++i) {
        if ((knownOpeners[i].m_line == virginLine && knownOpeners[i].m_startColumn > virginPos)
            || knownOpeners[i].m_line > virginLine) {
            return i;
        }
    }
...
template<class Trait>
inline void clearStyleValue(std::shared_ptr<Paragraph<Trait>> p,
                            int styleValue,
...
template<class Trait>
inline void setStyleValue(std::shared_ptr<Paragraph<Trait>> p,
                          int styleValue,
                          long long int startIndex,
                          long long int endIndex)
{
...
template<class Trait>
inline void dropCloser(std::shared_ptr<Paragraph<Trait>> p,
                       TextParsingOpts<Trait> &po,
                       const OpenerInfo &info)
{
    const auto addTextItem = [&po, &p](const StyleDelim &info,
                                       const typename Trait::String &style,
                                       long long int idx) -> std::shared_ptr<Text<Trait>> {
    ...
    const auto skipSameDelims = [&po](typename ItemWithOpts<Trait>::Styles &styles,
                                      const typename Trait::String &style,
                                      StyleDelim &info,
                                      long long int startIdx) {
        bool skip = true;
                            ...
                            long long int &closingDelimPos)
{
    ...
    if (itemWithOpts->type() == ItemType::Text) {
        ...
        po.m_rawTextData.insert(po.m_rawTextData.begin() + findRawTextIndex(p, info.m_index),
                                {style, pos.first, pos.second});
        ++index;
        ++rawTextIndex;
    }
...
inline long long int addOpenStyleDelimiter(std::shared_ptr<Paragraph<Trait>> p,
                                           ...
                                           long long int &closingDelimPos)
{
    const auto addOpenDelimiters =
        [&count, &style](ItemWithOpts<Trait> *was, ItemWithOpts<Trait> *where, long long int virginPos, bool append) {
            for (long long int i = 0; i < count; ++i) {
                if (append) {
                    where->openStyles().push_back({style, virginPos, was->startLine(), virginPos, was->startLine()});
                } else {
                    where->openStyles().insert(where->openStyles().begin() + i,
                                               {style, virginPos, was->startLine(), virginPos, was->startLine()});
                }
                ++virginPos;
            }
        };
    ...
    if (pos + count < rawText.m_str.length()) {
        ...
        if (pos > 0) {
            ...
            po.m_rawTextData.insert(po.m_rawTextData.begin() + rawIdx + 1,
        ...
        addOpenDelimiters(oldTextItem,
                          static_cast<ItemWithOpts<Trait> *>(p->items().at(open.m_index + 1).get()),
                          oldTextItem->startColumn(),
                          false);
...
inline long long int addCloseStyleDelimiter(std::shared_ptr<Paragraph<Trait>> p,
                                            ...
                                            long long int count)
{
    const auto addCloseDelims =
        [&start, &count, &style](ItemWithOpts<Trait> *was, ItemWithOpts<Trait> *become, bool prepend) {
            auto virginPos = was->startColumn() + start;

            for (long long int i = 0; i < count; ++i) {
                if (prepend) {
                    become->closeStyles().insert(become->closeStyles().begin() + i,
                                                 {style, virginPos, was->startLine(), virginPos, was->startLine()});
                } else {
                                           ...
                                           bool &finished)
{
    ...
    for (long long int i = 0; i < text.m_str.length(); ++i) {
        if (reverseSolidus.process(text.m_str[i])) {
            ...
            const bool punctBefore =
                (text.m_pos + i > 0 ? po.m_fr.m_data[text.m_line].first[text.m_pos + i - 1].isPunct()
                         || po.m_fr.m_data[text.m_line].first[text.m_pos + i - 1].isSymbol()
                                    : true);
            const bool uWhitespaceBefore =
                (text.m_pos + i > 0 ? Trait::isUnicodeWhitespace(po.m_fr.m_data[text.m_line].first[text.m_pos + i - 1])
                                    : true);
            ...
            const bool punctAfter = (text.m_pos + i < po.m_fr.m_data[text.m_line].first.length()
                                         ? po.m_fr.m_data[text.m_line].first[text.m_pos + i].isPunct()
                                             || po.m_fr.m_data[text.m_line].first[text.m_pos + i].isSymbol()
                                         : true);
            const bool uWhitespaceAfter =
                (text.m_pos + i < po.m_fr.m_data[text.m_line].first.length()
                     ? Trait::isUnicodeWhitespace(po.m_fr.m_data[text.m_line].first[text.m_pos + i])
                     : true);
                                     ...
                                     long long int &rawTextIndex)
{
    ...
    const auto closeOpeners = [&knownOpeners, &openers, &po, &p](const typename ItemWithOpts<Trait>::Styles &styles,
                                                                 const OpenerInfo &firstOpener) {
        if (!styles.empty()) {
            if (firstOpener.m_line != -1) {
                ...
                for (long long int i = 0; i < static_cast<long long int>(openers.size()); ++i) {
                    ...
                    const auto virginPos = po.m_fr.m_data[o.m_line].first.virginPos(
                        po.m_rawTextData[findRawTextIndex(p, o.m_index)].m_pos + o.m_startColumn);
                    const auto virginLine = po.m_fr.m_data[o.m_line].second.m_lineNumber;

                    if (virginLine > firstOpener.m_line
                        || (virginLine == firstOpener.m_line && virginPos > firstOpener.m_endColumn)) {
                        openerIdx = i;
    ...
    case ItemType::Text: {
        ...
        const auto idx =
            processEmphasisInText(p, po, index, knownOpeners, openers, delimiter, style, rawTextIndex, finished);
