...
TEST_CASE("001")
{
    ...
    REQUIRE(c->text() == TRAIT::latin1ToString("foo\tbaz\t\tbim"));
}
...
TEST_CASE("003")
{
    ...
    REQUIRE(c->isInline() == false);
    REQUIRE(c->syntax().isEmpty());
    REQUIRE(c->text() == TRAIT::utf8ToString(u8"a\ta\nὐ\ta"));
}
...
TEST_CASE("005")
{
    ...
    REQUIRE(c2->text() == TRAIT::latin1ToString("  bar"));
}
...
TEST_CASE("009")
{
    ...
    REQUIRE(t3->text() == TRAIT::latin1ToString("baz"));
}
...
#endif
        + TRAIT::latin1ToString("/tests/commonmark/0.30/010.md");
