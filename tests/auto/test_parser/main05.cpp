...
TEST_CASE("121")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/121.md"));
...
TEST_CASE("122")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/122.md"));
    ...
    REQUIRE(!m->isInline());
    REQUIRE(m->expr()
            == TRAIT::latin1ToString("\\[\\mathrm{\\mathbf{M}}(\\alpha) =\n"
                                     "   \\left(\n"
                                     "      \\begin{matrix}\n"
...
TEST_CASE("123")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/123.md"));
...

    {
        ...
        REQUIRE(t->text() == TRAIT::latin1ToString("List"));
...
TEST_CASE("124")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/124.md"));
    ...
    REQUIRE(p->items().at(0)->type() == MD::ItemType::Text);
    auto t = static_cast<MD::Text<TRAIT> *>(p->items().at(0).get());
    REQUIRE(t->text() == TRAIT::latin1ToString("Why is implementation of the following method so terrible?"));
    REQUIRE(t->startColumn() == 2);
    REQUIRE(t->startLine() == 0);
    ...
    REQUIRE(c->text()
            == TRAIT::latin1ToString("template< class Trait >\n"
                                     "inline void\n"
                                     "Parser< Trait >::parse( StringListStream< Trait > & stream,\n"
                                     "    std::shared_ptr< Block< Trait > > parent,\n"
                                     "    std::shared_ptr< Document< Trait > > doc,\n"
    ...
    REQUIRE(item->items().at(1)->type() == MD::ItemType::Paragraph);
    {
        ...
        REQUIRE(p->items().at(0)->type() == MD::ItemType::Text);
        auto t = static_cast<MD::Text<TRAIT> *>(p->items().at(0).get());
        REQUIRE(t->text() == TRAIT::latin1ToString("Implementation of the above method is so ugly because"));
        REQUIRE(t->startColumn() == 3);
...
TEST_CASE("125")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/125.md"));
...

    {
        ...
        REQUIRE(l->text() == TRAIT::latin1ToString("Issues"));
        REQUIRE(l->url() == TRAIT::latin1ToString("#issues/") + wd + TRAIT::latin1ToString("/125.md"));
    ...
    REQUIRE(h->text() == TRAIT::latin1ToString("<!-- endToc -->"));
...
TEST_CASE("126")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/126.md"));
...

    {
...

        {
            ...
            REQUIRE(p->items().at(0)->type() == MD::ItemType::Text);
            auto t = static_cast<MD::Text<TRAIT> *>(p->items().at(0).get());
            REQUIRE(t->text() == TRAIT::latin1ToString("Radiated 1G-6G"));
            REQUIRE(t->startColumn() == 2);
            REQUIRE(t->startLine() == 0);
...

    {
        ...
        REQUIRE(t->opts() == MD::TextOption::TextWithoutFormat);
        REQUIRE(t->text() == TRAIT::latin1ToString("Measured in GTEM using 12 Faces Method."));
        REQUIRE(t->startColumn() == 0);
        REQUIRE(t->startLine() == 1);
...

    {
...

        {
            ...
            REQUIRE(p->items().at(0)->type() == MD::ItemType::Text);
            auto t = static_cast<MD::Text<TRAIT> *>(p->items().at(0).get());
            REQUIRE(t->text() == TRAIT::latin1ToString("Results"));
            REQUIRE(t->startColumn() == 3);
...

    {
        ...
        REQUIRE(i->text() == TRAIT::latin1ToString("Measurement Result"));
        REQUIRE(i->url() == TRAIT::latin1ToString("MeasurementResult"));
...

    {
        ...
        REQUIRE(h->text() == TRAIT::latin1ToString("<Peaklist>\n<bar>"));
        REQUIRE(h->startColumn() == 0);
...

    {
...

        {
            ...
            REQUIRE(p->items().at(0)->type() == MD::ItemType::Text);
            auto t = static_cast<MD::Text<TRAIT> *>(p->items().at(0).get());
            REQUIRE(t->text() == TRAIT::latin1ToString("Observation"));
            REQUIRE(t->startColumn() == 3);
...

    {
        ...
        REQUIRE(t->opts() == MD::TextOption::TextWithoutFormat);
        REQUIRE(t->text() == TRAIT::latin1ToString("No relevant emissions other than radio transmissions."));
        REQUIRE(t->startColumn() == 0);
...

    {
...

        {
            ...
            REQUIRE(p->items().at(0)->type() == MD::ItemType::Text);
            auto t = static_cast<MD::Text<TRAIT> *>(p->items().at(0).get());
            REQUIRE(t->text() == TRAIT::latin1ToString("Images"));
            REQUIRE(t->startColumn() == 3);
...
TEST_CASE("127")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/127.md"));
...
TEST_CASE("128")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/128.md"));
...

    {
        ...
        REQUIRE(t->opts() == MD::TextOption::TextWithoutFormat);
        REQUIRE(t->text() == TRAIT::latin1ToString("full example"));
        REQUIRE(t->startColumn() == 2);
        REQUIRE(t->startLine() == 0);
        REQUIRE(t->endColumn() == 13);
        REQUIRE(t->endLine() == 0);
...

    {
        ...
        REQUIRE(!c->isInline());
        REQUIRE(c->text() == TRAIT::latin1ToString("#include <iostream>\n\nint main() { return 0; }"));
...
TEST_CASE("129")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/129.md"));
...
TEST_CASE("130")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/130.md"));
...
TEST_CASE("131")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/131.md"));
    ...
    REQUIRE(b->endColumn() == 1);
    REQUIRE(b->endLine() == 2);
    REQUIRE(b->delims() == MD::Blockquote<TRAIT>::Delims{{0, 0, 0, 0}, {0, 1, 0, 1}, {0, 2, 0, 2}});
...
TEST_CASE("132")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/132.md"));
...
TEST_CASE("133")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/133.md"));
...

    {
        ...
        REQUIRE(l->url() == TRAIT::latin1ToString("#files-in-workdata-too/") + wd + TRAIT::latin1ToString("/133.md"));
...
TEST_CASE("134")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/134.md"));
    ...
    REQUIRE(dt->text() == TRAIT::latin1ToString("Text"));
...
TEST_CASE("135")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/135.md"));
    ...
    REQUIRE(c->text() == TRAIT::latin1ToString("Text | Column 1 |"));
...

    {
        ...
        REQUIRE(dt->opts() == MD::TextOption::TextWithoutFormat);
        REQUIRE(dt->text() == TRAIT::latin1ToString("| -------- |"));
        REQUIRE(dt->startColumn() == 0);
...
TEST_CASE("136")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/136.md"));
...
TEST_CASE("137")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/137.md"));
...
TEST_CASE("138")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/138.md"));
...

    {
        ...
        REQUIRE(t->opts() == MD::TextOption::TextWithoutFormat);
        REQUIRE(t->text() == TRAIT::latin1ToString("Monday"));
        REQUIRE(t->startColumn() == 3);
        REQUIRE(t->startLine() == 1);
        REQUIRE(t->endColumn() == 8);
        REQUIRE(t->endLine() == 1);
...

    {
        ...
        REQUIRE(t->opts() == MD::TextOption::TextWithoutFormat);
        REQUIRE(t->text() == TRAIT::latin1ToString("fgh"));
        REQUIRE(t->startColumn() == 3);
        REQUIRE(t->startLine() == 3);
...

    {
        ...
        REQUIRE(t->opts() == MD::TextOption::TextWithoutFormat);
        REQUIRE(t->text() == TRAIT::latin1ToString("Tuesday"));
        REQUIRE(t->startColumn() == 0);
        REQUIRE(t->startLine() == 9);
        REQUIRE(t->endColumn() == 6);
        REQUIRE(t->endLine() == 9);
...

    {
        ...
        REQUIRE(t->opts() == MD::TextOption::TextWithoutFormat);
        REQUIRE(t->text() == TRAIT::latin1ToString("Wednesday"));
        REQUIRE(t->startColumn() == 0);
...
TEST_CASE("139")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/139.md"));
...
TEST_CASE("140")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/140.md"));
...
TEST_CASE("141")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/141.md"));
    ...
    REQUIRE(t->text()
            == TRAIT::latin1ToString("(settings: Dark style (left), Light style (right) "
...
TEST_CASE("142")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/142.md"));
...
TEST_CASE("143")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/143.md"));
...
TEST_CASE("144")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/144.md"));
...
TEST_CASE("145")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/145.md"));
...
TEST_CASE("146")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/146.md"));
...
TEST_CASE("147")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/147.md"));
...
TEST_CASE("148")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/148.md"));
...

    {
        ...
        REQUIRE(p->items().at(1)->type() == MD::ItemType::Text);
        auto s = static_cast<MD::Text<TRAIT> *>(p->items().at(1).get());
        REQUIRE(s->startColumn() == 17);
        REQUIRE(s->startLine() == 0);
        REQUIRE(s->endColumn() == 17);
        REQUIRE(s->endLine() == 0);
        REQUIRE(s->opts() == MD::TextWithoutFormat);
        REQUIRE(s->text() == TRAIT::latin1ToString(" "));
...
TEST_CASE("149")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/149.md"));
...
TEST_CASE("150")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/150.md"));
...
TEST_CASE("151")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/151.md"));
...
TEST_CASE("152")
{
    ...
    auto doc = parser.parse(TRAIT::latin1ToString("tests/parser/data/152.md"));
